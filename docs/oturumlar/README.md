...
| M1 — i18n: TR/EN anahtar eşliği doğrulaması (zaten 957/957) + `kontrol-dil.sh`'ın bu proje için KAPSAM DIŞI olduğunun kaydı + bilinen-desen (D-219 sınıfı: `.toLocaleUpperCase`/tarih biçimlendirme/sıralama) sınırlı bir elle grep taraması | `docs/oturumlar/M1-i18n-tarama.md` | **TAMAMEN BİTTİ (D-235+D-236, 2026-09-08/09).** §2.1 (kalıcı `src/i18n/localeParity.test.ts`, 957/957 eşliğini korur, mutation-verified) BİTTİ; §2.2'nin `.toFixed()` decimal-separator sorusu Barış'a koordinatör aracılığıyla soruldu ve cevaplandı ("altısı da düzeltilsin") — **P-67 KAPANDI (D-236)**, altı çağrı sitesi `Intl.NumberFormat(i18n.language, …)`'a geçirildi (export grafikleri `project.meta.language`, UI bileşenleri `i18n.language` — D-43/P-42'nin içerik-dili/UI-dili ayrımı korundu). |
| M2 — Auto-update: repo'yu public yapma (kendi onayıyla) + `tauri-plugin-updater` kurulumu + CI'ye release-publish adımı + Settings'e "check for updates" UI'si | `docs/oturumlar/M2-auto-update.md` | **KOD/CI/ALTYAPI TAM BİTTİ (D-238, 2026-09-09)** — gerçek uçtan uca akış (ilk `v*` tag push'u) Barış'ın kendi onayını bekliyor. Repo public (Barış kendisi yaptı — bu oturumun kendi `gh repo edit` denemesi ve bir kendi-kendine-izin-genişletme denemesi ikisi de Claude Code'un auto-mode sınıflandırıcısı tarafından blokland, D-46 CLOSED). `tauri-plugin-updater`/`tauri-plugin-process` (Rust+TS, gerçek crates.io/npm sürümleriyle) kuruldu, gerçek `tauri-plugin-updater`/`tauri-utils` kaynağı okunarak `pubkey` encoding'i ve `createUpdaterArtifacts`'ın `false` varsayılanı doğrulandı (mevcut imzasız `ci.yml` etkilenmiyor); `release/tauri.release.conf.json` (yalnızca release job'unda merge edilir) + yeni `.github/workflows/release.yml` (tag-tetiklemeli, `tauri-action`, gerçek kaynağı okunarak `tagName`'in otomatik düşmediği bulundu ve düzeltildi). Gerçek bir `tauri signer generate` anahtar çifti üretildi, public key config'e gömüldü, private key `gh secret set` ile kaydedildi. `src/updates/` (yeni: `updateIpc.ts`+`useUpdateCheck.ts`), `LaunchScreen`'in sessiz on-launch kontrolü + banner'ı, `SettingsScreen`'in kalıcı "Güncellemeler" bölümü. D-44'ün CI-mechanism yarısı burada çözüldü. |
| M3 — Packaging/signing kapanışı: D-44/D-46'yı "kalıcı imzasız, iç-kurumsal" durum olarak kapatan küçük bir kod+SÜREÇ dilimi (bundle metadata doğrulaması + sınırlı bir SmartScreen/Gatekeeper SÜREÇ yürüyüşü) | `docs/oturumlar/M3-paketleme-imza-kapanisi.md` | **KOD TARAFI TAM BİTTİ (D-231, 2026-09-08/09, ayrı worktree/branch)** — `shortDescription`/`publisher`/`copyright`/`LICENSE` dördü de dolduruldu. LICENSE + `publisher` kimlik-bağlı ikisi bu oturumun ortamında `AskUserQuestion` aracı bulunmadığı için önce iki aday cevapla birlikte bilinçli BOŞ bırakıldı; Barış'ın cevabı (Farplas Otomotiv A.Ş., ikisi de) geldiğinde uygulandı (bkz. D-231). SÜREÇ yürüyüşü (Gatekeeper/SmartScreen) bu ortamda yapılamadı, Barış'tan owed, P-12'yi kapatmıyor. |
| M4 — Polish: CLAUDE.md'nin kendi Quality floor listesinin (klavye nav, focus ring, WCAG AA kontrast, reduced motion, layout shift, unhandled rejection, console noise) proje çapında ilk sistematik denetimi | `docs/oturumlar/M4-polish-kalite-tabani.md` | **BİTTİ (D-237, 2026-09-09).** Doğrulama+küçük-kapatma turu, hata avı değil — taban zaten sağlamdı. Asıl bulgu: `a3PreviewWindow/window.ts`'in `listenForDescriptorPush`'ı + `LaunchScreen.tsx`'in iki dialog çağrısı + `SettingsScreen.tsx`'in iki mount-effect'i, hepsi D-134/D-136'nın kendi unhandled-rejection bug sınıfının yeni örnekleri — hepsi düzeltildi, mutation-doğrulandı. Küçük bir reduced-motion ihlali (`A3PreviewReservedBand.tsx`) düzeltildi; aynı dosyada gerçek, mekanik doğrulanmış bir CLS kaynağı yalnızca yumuşatıldı (kökten çözüm BVVL bekliyor). Klavye-nav statik taraması sıfır yeni ihlal buldu. WCAG AA kontrast — gerçek, büyük bir ihlal (`--color-fp-*` token'ları, D-218/D-219) ayrı **P-68** olarak dosyalandı, bu dilime karıştırılmadı. Owed: gerçek klavye yürüyüşü, gerçek CLS DevTools ölçümü, P-68'in kendi BVVL turu. |
...
**P-67 KAPANDI (D-236)** — M1'in kendi `.toFixed()` decimal-separator sorusu Barış'a soruldu,
cevaplandı, uygulandı. **P-68** (yeni — M4'ün kendi denetiminde bulundu, `--color-fp-*`
token'larının WCAG AA kontrast başarısızlığı, ışık VE koyu temada, birden fazla rolde —
kendi BVVL turunu bekliyor).
...
**§0 taraması + kanıt toplama BİTTİ 2026-09-08 (D-232) — kapsam kararı kendisi PENDING
...
| Export/UI sadakat gedikleri | `docs/oturumlar/export-ui-sadakat-gedikleri.md` | P-17 (border fidelity), P-18 (header/footer alanları bağlı değil), P-22 (action plan Gantt), P-23 (6a editörlerinin FieldFormEditor'e taşınması), P-27 (impact-effort 2×2 drag-drop), P-42 (Fishbone kategori etiketleri yanlış dile bağlı — gerçek bug, en ucuz madde) | **TAMAMEN İŞLENDİ** — P-42 KAPANDI (D-261, statik bilingual sözlük deseni), P-23 KISMEN KAPANDI (D-262, yalnızca `fiveW2H`; `gapStatement`/`problemTypeClassifier`/`msaGageRr` gerçek şema-tipi engelleriyle kalıcı kapsam dışı), P-17 oturumdan çıkarıldı (Barış görmedi), P-18 KAPANDI (WONTFIX, D-263). **P-27 TAMAMEN KAPANDI (D-269, `P27-impact-effort-drag-drop.md`'nin kendi oturumunda)** — Barış'ın kendi `AskUserQuestion` seçimiyle "gerçek inşa et"e döndü: gerçek serbest-pozisyon 2×2 drag-drop canvası (`ImpactEffortCanvas.tsx`, skor birincil kaynak kalıyor, sürükleme yalnızca giriş yöntemi) + Barış'ın önerilenin AKSİNE seçtiği gerçek bir 2×2 scatter export grafiği (`ImpactEffortChart.tsx`, yeni `A3ImageKind`). **P-22 de TAMAMEN KAPANDI (D-270, `P22-action-plan-gantt.md`'nin kendi oturumunda)** — kod öncesi bulunan mimari çatallanma iki `AskUserQuestion` turuyla çözüldü: Barış Seçenek B'yi (gerçek blok-seviyesi agregasyon) seçti, sonra grafik+metin yerleşimi soruldu (Recommended: grafik üstte, metin altta). Yeni `A3BlockAggregateImage` mekanizması `buildA3Layout.ts`'in blok döngüsüne entegre edildi; `elasticAllocation.ts`'in demand-tahmini de aynı oturumda düzeltildi (grafiğin `rowSpan`'i artık hesaba katılıyor, yoksa `pps-8step-auto`'nun elastic Step 6 bloğu sessizce eksik büyüyecekti). Her ikisi de artık **tamamen kapalı** — bu satırın kendi altı maddesinin (P-17/P-18/P-22/P-23/P-27/P-42) hepsi işlendi. |
| Şema/veri modeli kararları | `docs/oturumlar/sema-veri-modeli-kararlari.md` | P-39 (Why-Why node-seviyesi referans, D-185'te zaten karar verildi, inşa edilmedi), P-41 (Fishbone↔5-Why cross-linking tasarım sorusu), P-66 (tpmLossTaxonomy 7 vs 8 kategori, migration gerektirir) | **TAMAMEN BİTTİ — üçü de KAPANDI.** P-41 KAPANDI, her iki yarısı da (D-265: cross-linking gerekmiyor; D-266: Step 4 tier sırası five-why→why-why-tree→fishbone olarak GERÇEKTEN uygulandı, `registry.ts`, mutation-verified). **P-66 TAMAMEN KAPANDI (D-267, `P66-tpmLossTaxonomy-8-kategori-migration.md`'nin kendi oturumunda)** — migrasyon stratejisi soruldu (Seçenek c, ikisine de kopyala) ve kod yazıldı: `TPM_LOSS_CATEGORIES` 8 elemana çıktı, `v1ToV2SplitTpmMaintenance` migration'ı (schema v1→2, D-57'nin zincirine ilk gerçek ekleme) canlı. **P-39 TAMAMEN KAPANDI (D-268, `P39-node-seviyesi-referans.md`'nin kendi oturumunda)** — kendi launch prompt'u zaten mekanik bir inşa turuydu, tasarım sorusu yoktu: `targetNodeId?` şeması, üç selector'ün node-farkındalığı, `EntryReferenceField`'ın ikinci seviyesi (`WhyWhyTreeNodePicker`), ve D-129'un deseniyle bir gerçek Rust round-trip testi hepsi inşa edildi, hepsi mutation-verified. |
| P-71 (P-60'ın kalan kablolaması) | `docs/oturumlar/P71-cost-log-korelasyon-kablolama.md` | K1 (`LayoutReviewPanel`), K3 field-level (`EntryTranslateField`), K3 whole-report (`TranslateReportPanel`), D-247 (`AssistantPanel`'in "Apply the suggestion" akışı) — dördü de `markAccepted`'ı henüz çağırmıyor; mekanizmanın kendisi D-260'ta zaten inşa edildi, bu yalnızca aynı deseni dört yerde tekrarlamak + iki küçük gerçek tasarım kararı (çok-satırlı diff'lerde "accepted" ne demek, D-247'nin hangi çağrısı korelasyona girer) | Başlamadı — kendi launch prompt'u yazıldı, 2026-09-15 |
...
**P-64 CLOSED 2026-09-09 (D-233)**, ayrı bir küçük dilimde — `PinnedBlockSummary.tsx` her
kolonun SON bloğu için bir manuel sayısal giriş satırı (`ManualPinRow`) kazandı; `BlockPinOverlay`'in
kendi `n-1` sürükleme-tutamacı mekanizması bilerek DEĞİŞMEDİ. Tam kayıt: `DECISIONS.md` D-233,
`docs/oturumlar/kucuk-acik-maddeler.md` §2.4.
