...
| Şema/veri modeli kararları | `docs/oturumlar/sema-veri-modeli-kararlari.md` | P-39 (Why-Why node-seviyesi referans, D-185'te zaten karar verildi, inşa edilmedi), P-41 (Fishbone↔5-Why cross-linking tasarım sorusu), P-66 (tpmLossTaxonomy 7 vs 8 kategori, migration gerektirir) | **KARAR TURU BİTTİ (2026-09-15).** P-41 KAPANDI, her iki yarısı da (D-265: cross-linking gerekmiyor; D-266: Step 4 tier sırası five-why→why-why-tree→fishbone olarak GERÇEKTEN uygulandı, `registry.ts`, mutation-verified). P-66 kararı verildi ama kodlanmadı (D-264: 8 kategoriye geç, migration ile — Barış önerilenin AKSİNE seçti); kendi launch prompt'u `P66-tpmLossTaxonomy-8-kategori-migration.md` (migration stratejisi hâlâ sorulması gerekiyor). P-39 yeniden doğrulandı (P-41 ile birleşme fırsatı gerçekleşmedi, bağımsız kalıyor); kendi launch prompt'u `P39-node-seviyesi-referans.md` (artık salt mekanik bir inşa turu). |
