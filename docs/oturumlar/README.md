...
| Şema/veri modeli kararları | `docs/oturumlar/sema-veri-modeli-kararlari.md` | P-39 (Why-Why node-seviyesi referans, D-185'te zaten karar verildi, inşa edilmedi), P-41 (Fishbone↔5-Why cross-linking tasarım sorusu), P-66 (tpmLossTaxonomy 7 vs 8 kategori, migration gerektirir) | **KARAR TURU + P-66'nın kendi kod turu BİTTİ.** P-41 KAPANDI, her iki yarısı da (D-265: cross-linking gerekmiyor; D-266: Step 4 tier sırası five-why→why-why-tree→fishbone olarak GERÇEKTEN uygulandı, `registry.ts`, mutation-verified). **P-66 TAMAMEN KAPANDI (D-267, `P66-tpmLossTaxonomy-8-kategori-migration.md`'nin kendi oturumunda)** — migrasyon stratejisi soruldu (Seçenek c, ikisine de kopyala) ve kod yazıldı: `TPM_LOSS_CATEGORIES` 8 elemana çıktı, `v1ToV2SplitTpmMaintenance` migration'ı (schema v1→2, D-57'nin zincirine ilk gerçek ekleme) canlı. P-39 yeniden doğrulandı (P-41 ile birleşme fırsatı gerçekleşmedi, bağımsız kalıyor); kendi launch prompt'u `P39-node-seviyesi-referans.md` (artık salt mekanik bir inşa turu). |
