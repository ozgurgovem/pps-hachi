...
kolonun SON bloğu için `ManualPinRow` adlı uncontrolled bir sayısal giriş satırı kazandı;
`BlockPinOverlay.tsx`'in kendi `n-1` sürükleme-tutamacı mekanizması bilerek DEĞİŞMEDİ (§2.1'in
kendi kapsam sınırı hâlâ geçerli — bu yalnızca son bloğu pinlemenin İKİNCİ, tutamaç-dışı bir
yolu). `elasticAllocation.test.ts` gerçekten dokunulmadı (§2.3'ün kendi uyarısına uyuldu).
