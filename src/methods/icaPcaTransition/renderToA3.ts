...
export function renderIcaPcaTransitionToA3(
  ...
  const values = {
    ...payload,
    status: status.length > 0 ? (ICA_PCA_STATUS_EXPORT_LABELS[status]?.[language] ?? status) : status,
  };
  ...
  return {
    lines: [titleLine, ...fieldFormLines(values, ICA_PCA_TRANSITION_FIELDS, language)],
  };
}
