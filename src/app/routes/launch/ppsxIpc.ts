...
export interface ImportImageResult {
  imageRef: ImageRefResult;
  /** Fold both into the caller's `otherEntries` — see `image_import`'s own Rust-side doc comment. */
  originalEntry: ArchiveEntryPayload;
  thumbnailEntry: ArchiveEntryPayload;
  modifiedMs: number;
}

/**
 * D-118/D-193: reads `sourcePath` itself in Rust — the raw photo's bytes
 * never cross into TypeScript at all, only the small `ImageRefResult` +
 * capped-size processed bytes come back. `imageId` is generated by the
 * caller (`crypto.randomUUID()`) so the resulting `ImageRef.id` is known
 * before the call returns. This is also a full, immediate project save
 * (same `manifest`/`project`/`otherEntries`/`expectedModifiedMs` shape as
 * `writePpsx`) — D-118: "writes... immediately," not merely image
 * processing.
 */
export async function importImage(
  ...
  try {
    return await invoke<ImportImageResult>("image_import", {
      ppsxPath,
      sourcePath,
      imageId,
      manifest,
