...
export type Attempt =
  | { readonly success: true; readonly value: unknown; readonly requestId: string }
 ...
 *
 * P-60 (ai-katmani-temizligi.md §4): generates one `crypto.randomUUID()`
 * per physical call and passes it into `completeStructured` — Rust writes
 * it into the log entry it appends. Returned on success (`Attempt.requestId`)
 * so a caller that wants to correlate a later Accept/Reject
 * (`settingsIpc.ts`'s `markAccepted`) has it in hand; callers that don't
 * care simply never read the field. Not generated by the caller: this keeps
 * every existing call site's own signature untouched (Anayasa Madde 1 —
 * the smallest change that gets the mechanism built).
 */
export async function attemptStructuredProposal(
  ...
  let raw: unknown;
  try {
    raw = await completeStructured(prompt, jsonSchema, modelId, redaction, projectId, promptVersion, requestId);
  } catch (error) {
  ...
  if (parsed.success) {
    return { success: true, value: parsed.data, requestId };
  }
  return {
    success: false,
...
export type ProposalResult =
  | { readonly outcome: "success"; readonly value: unknown; readonly requestId: string }
...
export async function proposeStructuredEntry(params: ProposeStructuredEntryParams): Promise<ProposalResult> {
  ...
  if (first.success) {
    return { outcome: "success", value: first.value, requestId: first.requestId };
  }
  ...
  if (second.success) {
    return { outcome: "success", value: second.value, requestId: second.requestId };
  }
